...
i=0
for item in mods1:
    print("\"Primary\": " + item.find_all("span", {"class": "statmod-stat-label"})[0].text)
...
for item in mods1:
    ...
    while (j < 4):
        ...
        if label == "Speed":
            print("\"Spd\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        elif label == "Critical Chance" and value.endswith("%"):
            print("\"Crit_%\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        elif label == "Offense" and value.endswith("%"):
            print("\"Off_%\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        elif label == "Offense":
            print("\"Off\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        elif label == "Protection" and value.endswith("%"):
            print("\"Prot_%\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        elif label == "Protection":
            print("\"Prot\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        elif label == "Health" and value.endswith("%"):
            print("\"HP_%\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        elif label == "Health":
            print("\"HP\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        elif label == "Defense" and value.endswith("%"):
            print("\"Def_%\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        elif label == "Defense":
            print("\"Def\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        elif label == "Potency" and value.endswith("%"):
            print("\"Pot_%\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        elif label == "Tenacity" and value.endswith("%"):
            print("\"Ten_%\":" + mods2[i].find_all("span", {"class": "statmod-stat-value"})[j].text.replace('+', '').replace('%', '') + ",")
        j += 1
    i += 1
